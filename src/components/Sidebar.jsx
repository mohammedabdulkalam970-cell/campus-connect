...
import {
    FiHome, FiBook, FiCalendar, FiSearch, FiMessageSquare,
    FiUser, FiLogOut, FiShield, FiMenu, FiX, FiSun, FiMoon
} from 'react-icons/fi';
    ...
    const renderSidebarContent = (isMobile = false) => (
        <div className="flex flex-col h-full">
            ...
            <div className={`flex items-center gap-3 p-4 border-b border-white/10 ${collapsed && !isMobile ? 'justify-center' : ''}`}>
                ...
                {(!collapsed || isMobile) && (
                    <div>
                        <p className="text-white font-bold text-sm leading-tight">Campus Connect</p>
                        <p className="text-blue-300 text-xs">NECN</p>
                    </div>
                )}
            </div>
            ...
            <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
               ...
               ${isActive ? 'bg-amber-500/30 text-amber-300 font-semibold' : 'text-amber-400/70 hover:bg-amber-500/20 hover:text-amber-300'}
               ${collapsed && !isMobile ? 'justify-center' : ''}`
                        }
                    >
                        <FiShield className="w-4 h-4 flex-shrink-0" />
                        {(!collapsed || isMobile) && <span className="text-sm">Admin Panel</span>}
                    </NavLink>
                )}
            </nav>
            ...
            <div className="p-3 border-t border-white/10 space-y-1">
                ...
                >
                    ...
                    {(!collapsed || isMobile) && <span className="text-sm">{isDark ? 'Light Mode' : 'Dark Mode'}</span>}
                </button>
                <button
